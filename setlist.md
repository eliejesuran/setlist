...
~~Le `×` supprimait un titre immédiatement et de façon irréversible.~~  
**Implémenté :** un toast apparaît en bas de l'écran pendant 4 secondes après chaque suppression (titre ou séparateur), avec un bouton **Annuler** doré qui restaure l'item à sa position exacte. Fonctionne aussi pour les séparateurs.
...
~~`openSheet()` ne permettait d'éditer que le `title`.~~  
**Implémenté :** le sheet affiche désormais 4 champs — **Titre**, **Artiste**, **Durée** (avec normalisation `M:SS` au blur, identique au comportement desktop), **Note scène** (sur la même ligne que la durée). Focus automatique sur le champ Titre à l'ouverture.
...
**Problème :** en thème `light-paper` ou `light-clean`, la barre d'outils (`.toolbar`) garde un fond quasi-noir (`rgba(10,10,10,.97)`) hardcodé. Les boutons et badges restent eux aussi en couleurs dark, illisibles sur fond clair.  
**Fix proposé :** remplacer le fond hardcodé de `.toolbar` par `background:var(--card)` (ou une variable dédiée), et s'assurer que les couleurs des `.tbtn` et `.hbadge` héritent des variables CSS thémées.
...
**Problème :** `html2canvas` capture à `scale:2` en PNG non compressé. Une page A4 à ×2 = ~1588×2246px → PNG ~14 Mo. Le PDF final peut dépasser 50 Mo pour 4 pages.  
**Pistes de réduction :**
- Passer de `image/png` à `image/jpeg` avec qualité 0.85–0.92 (sauf pour le mode dark où les aplats noirs compressent très bien en JPEG)
- Réduire `scale` de `2` à `1.5` — qualité encore très correcte à l'impression A4, taille divisée par ~1.8
- Combiner les deux : JPEG + scale 1.5 → gain attendu ×5 à ×8 (≈ 2 Mo/page)
- Pour le mode `light`, JPEG est sans perte visuelle perceptible ; pour `dark`, tester visuellement les artefacts
...
**Problème :** le bouton `saveHtml()` est rarement utilisé, encombre la toolbar, et la sauvegarde HTML via regex est fragile (voir B2).  
**Fix proposé :** supprimer le bouton et son listener. La persistance est assurée par le localStorage (📌), suffisant pour l'usage courant.

#### U6 — Thème : remplacer le panneau par un toggle Dark/Light
**Problème :** le panneau thème à 4 options est surdimensionné pour un besoin simple. L'usage réel se résume à dark (scène) et light (impression/répétition).  
**Fix proposé :**
- Garder uniquement `dark-gold` et `light-paper`
- Remplacer le bouton 🌈 par un petit bouton toggle `☀ / 🌙` dans la toolbar
- Le PDF s'imprime dans le thème actif (dark → fond noir, light → fond blanc) — supprimer les deux boutons PDF séparés et les remplacer par un seul **"⬇ PDF"**
- **Attention B6 :** corriger le fond hardcodé de la toolbar avant d'activer ce changement

#### U7 — Fusion Import / Export en un seul overlay
**Problème :** deux boutons et deux overlays distincts pour import et export, alors que le flux naturel est : exporter → modifier → réimporter.  
**Fix proposé :**
- Un seul bouton **"📋 Texte"** ouvre un overlay unique
- À l'ouverture : la textarea est pré-remplie avec l'export actuel (badges + titres)
- Si l'utilisateur modifie le texte, un bouton **"↺ Appliquer"** remplace la setlist (mode remplace uniquement — l'append n'est pas nécessaire puisque le texte d'export est déjà présent)
- Bouton **"Copier"** toujours disponible pour copier sans modifier
- Résultat : −1 bouton, −1 overlay, UX plus fluide
