...
**Bouton `+ Nouvelle session`** : crée une setlist vide en mode offline, déconnecte la session active. Peut être mise en ligne ensuite via `🔗 Partager`.
...
**Indicateur de statut :** rond dans la toolbar — gris (offline), orange (connexion), vert (connecté). Nom du membre affiché dans le bouton quand connecté.
...
**Problème :** les modifications de badges ne déclenchent `wsPatch()` que si une autre modification de la setlist survient ensuite. `renderBadges()` ne passe pas par le `renderList()` surchargé.  
**Fix :** appeler `wsPatch()` directement à la fin de `renderBadges()`.
...
**Problème :** ces boutons sont redondants voire trompeurs quand une session WS est active — la source de vérité est le serveur.  
...
| Éditer titre / champs privés | Clic sur le numéro de la chanson → sheet |
| Éditer titre (mobile) | Tap sur le titre → sheet |
...
Lignes 47 et 49 : `.import-box` → `var(--card)` / `var(--border)` ; `.import-ta` → `var(--black)` / `var(--white)` / `var(--border)`.
...
Deux protections ajoutées dans `server.js` : origin check (connexions acceptées uniquement depuis `eliejesuran.github.io` et localhost) + rate limiting (max 10 connexions/min/IP). Taille des messages limitée à 128 Ko.
...
Bouton `🔗 Partager` + bouton `+ Nouvelle session` dans la toolbar. Serveur WebSocket sur Fly.io (`wss://ssbbb-server.fly.dev`), Paris. Sessions 8 chars, TTL 8h, max 100 sessions / 20 peers. Nom du membre affiché dans le bouton toolbar quand connecté. Reconnexion auto toutes les 5s. URL `?s=abc12345` pour rejoindre directement.
...
Deux champs `privateUrl` et `privateNote` ajoutés à la structure `Song`. Accessibles via le sheet (clic sur le numéro de la chanson, desktop et mobile). Icône `🔗` discret affiché dans la ligne si `privateUrl` est renseigné. Champs exclus de `buildPdfHtml()` et du format texte d'échange. Synchronisés via WebSocket comme le reste de l'état.
...
*Documentation mise à jour le 18 mai 2026.*
