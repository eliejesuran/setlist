...
4. `html2canvas` capture chaque page — scale ×1.5 pour le mode dark, ×2 pour le mode light
5. `jsPDF` assemble les images — PNG pour les deux modes (light en JPEG qualité 1.0 conservé pour compatibilité)

**Nom de fichier généré :** `ssbbb_{badge1}_{badge2}_{mode}.pdf`

### 6. Thèmes

3 thèmes, bascule via le bouton **`☀ / 📜 / 🌙`** dans la toolbar (cycle Dark → Sépia → Light) :

| Clé | Icône | Description | PDF généré |
|---|---|---|---|
| `dark-gold` | ☀ | 🖤 Fond noir, or — thème par défaut (scène) | Fond noir, scale ×1.5 |
| `sepia` | 📜 | 📜 Ocre jauni, tons brun tabac — répétition | Fond blanc (mode light), scale ×2 |
| `light-paper` | 🌙 | ☁️ Blanc épuré — impression papier | Fond blanc, scale ×2 |
...
**Implémenté :** scale différencié par mode — `1.5` pour dark (aplats noirs, PNG compresse bien), `2` pour light/sépia (texte fin sur fond clair, qualité prioritaire). Mode light utilise JPEG qualité 1.0 ; mode dark reste en PNG.
...
**Implémenté :** 3 thèmes — `dark-gold`, `sepia` (ocre vintage), `light-paper`. Bouton toggle **`☀ / 📜 / 🌙`** en cycle dans la toolbar. Un seul bouton **`⬇ PDF`** qui génère dans le thème actif (sépia → rendu light). Dépendait de B6 (corrigé).
